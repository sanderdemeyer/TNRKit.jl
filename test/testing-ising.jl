...
for (i, T) in enumerate(Ts)
    scheme = BTRG(classical_ising(1/T), -0.5; stop=maxiter(100)&convcrit(1e-20, btrg_convcrit))
...
CSV.write("data/relerror.csv", DataFrame(Ts=Ts, fs_btrg=fs_btrg, fs_trg=fs_trg, fs_onsager=onsager_free_energy(Ts)))