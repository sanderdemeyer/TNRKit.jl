...
function step!(scheme::ATRG, trunc::TensorKit.TruncationScheme)
    U, S, V, _ = tsvd(scheme.T, ((1, 3), (2, 4)); trunc=trunc)

    @tensor A[-1 -2; -3] := U[-1 -3; -2]
    @tensor B[-1; -2 -3] := S[-2; 1] * V[1; -1 -3]
    @tensor C[-1 -2; -3] := U[-1 -3; 1] * S[1; -2]
    @tensor D[-1; -2 -3] := V[-2; -1 -3]

    @tensor M[-1 -2; -3 -4] := B[1; -3 -4] * C[-1 -2; 1]

    U1, S1, V1, _ = tsvd(M, ((1, 3), (2, 4)); trunc=trunc)

    @tensor X[-1 -2; -3] := U1[-1 -3; 1] * sqrt(S1)[1; -2]
    @tensor Y[-1; -2 -3] := sqrt(S1)[-2; 1] * V1[1; -1 -3]

    @tensor temp1[-1; -2 -3 -4] := D[-1; 1 -4] * Y[1; -2 -3]
    @tensor temp2[-1 -2 -3; -4] := A[-1 1; -4] * X[-2 -3; 1]

    _, R1 = leftorth(temp1, ((1, 2), (3, 4)))
    R2, _ = rightorth(temp2, ((1, 2), (3, 4)))

    @tensor temp[-1; -2] := R1[-1; 1 2] * R2[1 2; -2]
    ...
    @tensor H[-1; -2 -3] := D[-1; 1 2] * Y[1; -2 3] * Proj_1[3 2; -3]
    @tensor G[-1 -2; -3] := Proj_2[-1; 1 2] * X[2 -2; 3] * A[1 3; -3]

    @tensor scheme.T[-1 -2; -3 -4] := G[-1 1; -3] * H[-2; 1 -4]
