mutable struct VBTNR{E, S} <: TNRScheme
    T::TensorMap{E, S, 2, 2}
    finalize!::Function
...
mutable struct _vbtnr_horizontal_projector{E, S}
    mps::MPSKit.InfiniteMPS
    env::MPSKit.InfiniteEnvironments
    w::TensorMap{E, S, 2, 1}
    function _vbtnr_horizontal_projector(mps::MPSKit.InfiniteMPS, env::MPSKit.InfiniteEnvironments, χh::Int)
        w = randn(scalartype(mps), space(env.GLs[1], 2) ⊗ space(env.GLs[1], 2) ← spacetype(mps)(χh))
        return new{scalartype(w), spacetype(w)}(mps, env, w)
    end
end

function trΩ(x::_vbtnr_horizontal_projector)
...
function trΩ_w_dagw(x::_vbtnr_horizontal_projector)
...
function Γ(x::_vbtnr_horizontal_projector)
...
mutable struct _vbtnr_intermediate_projector_trbl{E, S}
    mps::MPSKit.InfiniteMPS
    env::MPSKit.InfiniteEnvironments
    Ttr::TensorMap{E, S, 1, 2}
    Tbl::TensorMap{E, S, 2, 1}
    w::TensorMap{E, S, 1, 1}
    function _vbtnr_intermediate_projector_trbl(mps::MPSKit.InfiniteMPS, env::MPSKit.InfiniteEnvironments, T::TensorMap{E, S, 2, 2}, χq::Int) where {E, S}
...
function Γ(x::_vbtnr_intermediate_projector_trbl)
...
mutable struct _vbtnr_intermediate_projector_tlbr{E, S}
    mps::MPSKit.InfiniteMPS
    env::MPSKit.InfiniteEnvironments
    Ttl::TensorMap{E, S, 1, 2}
    Tbr::TensorMap{E, S, 2, 1}
    w::TensorMap{E, S, 1, 1}
    function _vbtnr_intermediate_projector_tlbr(mps::MPSKit.InfiniteMPS, env::MPSKit.InfiniteEnvironments, T::TensorMap{E, S, 2, 2}, χq::Int) where {E, S}
...
function trΩ(x::_vbtnr_intermediate_projector_tlbr) # Done
    return @tensoropt x.env.GLs[1][1 2; 3] * x.mps.AC[1][3 4; 5] * x.env.GRs[1][5 8; 6] * x.Ttl[2; 4 9] * x.Tbr[9 7; 8] * conj(x.mps.AC[1][1 7; 6])
end

function trΩ_w_dagw(x::_vbtnr_intermediate_projector_tlbr) # Done
...
function Γ(x::_vbtnr_intermediate_projector_tlbr)
...
function run!(scheme::VBTNR, trunc::TensorKit.TruncationDimension, criterion::stopcrit; inner_trunc = trunc, VUMPSalg = VUMPS(), envdim::Int = trunc.dim)
    ...
    while crit
        ...
        vert = _vbtnr_vertical_projector(mps, trunc.dim)
        optimize!(vert, trunc; convcrit = 1.0e-9)

        horiz = _vbtnr_horizontal_projector(mps, env, trunc.dim)
        optimize!(horiz, trunc; convcrit = 1.0e-9)

        qstruct1 = _vbtnr_intermediate_projector_trbl(mps, env, scheme.T, inner_trunc.dim)
        optimize!(qstruct1, inner_trunc; convcrit = 1.0e-9)

        qstruct2 = _vbtnr_intermediate_projector_tlbr(mps, env, scheme.T, inner_trunc.dim)
...
const VBTNR_PROJECTOR = Union{_vbtnr_vertical_projector, _vbtnr_horizontal_projector, _vbtnr_intermediate_projector_trbl, _vbtnr_intermediate_projector_tlbr}
...
function _update_boundary(mps::MPSKit.InfiniteMPS, proj::_vbtnr_vertical_projector)
    return InfiniteMPS([@tensor AL′[-1, -2; -3] := mps.AL[1][-1; 1 2] * mps.AL[1][2 3; -3] * proj.w[-2; 1 3]])
end

function _update_environment(env::MPSKit.InfiniteEnvironments, proj::_vbtnr_horizontal_projector)
...
function _update_tensor!(scheme::VBTNR, vert::_vbtnr_vertical_projector, horiz::_vbtnr_horizontal_projector, qstruct1::_vbtnr_intermediate_projector_trbl, qstruct2::_vbtnr_intermediate_projector_tlbr)
